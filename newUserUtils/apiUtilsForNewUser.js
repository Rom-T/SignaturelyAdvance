    ...
    await step(`Free User Sign up`, async () => {
        ...
        await step(`Free user sign up API call`, async () => {
            while (attempt < maxRetries) {
                attempt++;
                response = await request.post(`${process.env.API_URL}${API_URL_END_POINTS.signUpEndPoint}`, {
                ...
                if (attempt === maxRetries) {
                    throw new Error(
                        `Failed to proceed User sign up after ${maxRetries} attempts: ${response.status()}`
                    );
                }
